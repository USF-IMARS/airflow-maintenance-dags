...
kill off tasks that are running in the background that don't correspond to
a running task in the DB.

This is useful because when you kill off a DAG Run or Task through the
Airflow Web Server, the task still runs in the background on one of the
executors until the task is complete.
...
# Whether to send out an email whenever a process was killed during a DAG
# Run or not
...
# Whether the job should delete the db entries or not. Included if you want
# to temporarily avoid deleting the db entries.
ENABLE_KILL = True
...
def kill_halted_tasks_function(**context):
    ...
    logging.info("process_killed_email_subject:   " + str(
        PROCESS_KILLED_EMAIL_SUBJECT
    ))
    logging.info("process_killed_email_addresses: " + str(
        PROCESS_KILLED_EMAIL_ADDRESSES
    ))
    ...
    for line in search_output_filtered:
        ...
        # apache-airflow version >= 1.10 requires datetime field values
        # with timezone
        ...
        if DEBUG:
            ...
            for dag_run in session.query(DagRun).filter(
                ...
                logging.info(
                    "DEBUG: dag_run: " + str(dag_run) +
                    ", dag_run.state: " + str(dag_run.state)
                )
            logging.info("")
        logging.info(
        ...
        # Checking to make sure the TaskInstance is available and
        # in a running state
        if DEBUG:
        ...
        if task_instance is None:
            kill_reason = (
                "Task Instance was not found in metastore. "
                "Marking process to be killed."
            )
            process["kill_reason"] = kill_reason
            processes_to_kill.append(process)
            logging.warn(kill_reason)
...
def branch_function(**context):
    logging.info(
        "Deciding whether to send an email about tasks that were "
        "killed by this DAG..."
    )
    logging.info(
    ...
    if processes_to_kill is not None and len(processes_to_kill) > 0:
        ...
        if ENABLE_KILL:
            ...
            logging.info(
                "Opting to send an email to alert the users that "
...
<html>
    <body>
        ...
        <table>
            ...
            <tr><td><b> Log URL: </b></td><td><a
                href="{{ task_instance.log_url }}">{{ task_instance.log_url }}
            </a></td></tr>
        </table>
